...
def run_3d_physics_test():
    ...
    frames = 500
    print(
        f"Mesh: {mesh.n_cells} cells | Layers: {p.n_layers} | Total State: {mesh.n_cells * p.n_layers} cells"
    )
    ...
    for frame in range(1, frames + 1):
        ...
        if frame % 50 == 0 or frame == frames:
            ...
            print(
                f"Step {frame:04d} | Thermal: {thermal:8.2f} | Horiz KE: {horiz_ke:7.2f} | Vert KE: {vert_ke:7.2f} | Max Wind: {max_wind:.3f} | Max Vert: {max_w:.3f}"
            )
