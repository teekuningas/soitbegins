...
def subdivide_project(positions, indices):
    ...
    for triag in indices:
        ...
        mp12 = np.array([(v1[0] + v2[0]) / 2, (v1[1] + v2[1]) / 2, (v1[2] + v2[2]) / 2])
        mp12 = mp12 / np.linalg.norm(mp12)

        mp13 = np.array([(v1[0] + v3[0]) / 2, (v1[1] + v3[1]) / 2, (v1[2] + v3[2]) / 2])
        mp13 = mp13 / np.linalg.norm(mp13)

        mp23 = np.array([(v2[0] + v3[0]) / 2, (v2[1] + v3[1]) / 2, (v2[2] + v3[2]) / 2])
...
if DRAW_RESULTS:
    ...
    V = np.zeros(len(vv), [("a_position", np.float32, 3), ("a_color", np.float32, 4)])
    V["a_position"] = vv
