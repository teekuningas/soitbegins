...
def run_mesh_diagnostics(subdivisions):
    ...
    print(f"Expected Verts:  {expected_verts} -> {'PASS' if mesh.n_cells == expected_verts else 'FAIL'}")
    ...
    print(f"Vertices with 6 neighbors (Hexagons):      {count_6} (Expected: {mesh.n_cells - 12})")
    if other > 0:
